    ...
    <div className="azkars-page">
      <section className="azkars">
        <div className="container text-center mt-5">
          <div className="title">
            <h1 className="mb-4">{azkar && azkar.length > 0 && azkar[0].category}</h1>
            <p className="h4 mb-4 ">عددها {azkar.length} ذكر</p>
          </div>
          <div className="row">
            {azkar &&
              azkar.slice(0, 30).map((azkar, i) => {
                return <Zekr azkar={azkar} index={i} key={i} handleDecrement={handleDecrement} />;
              })}
          </div>
        </div>
      </section>
